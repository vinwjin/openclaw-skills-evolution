...
## 四、skill_manager_tool.py actions
...
## 九、安装方式与 OpenClaw Plugin 机制

### 正确注册方式（来自 lossless-claw）
...
`kind` 可选值：
- `general` — 通用 plugin
- `context-engine` — 上下文压缩引擎（lossless-claw 使用）

`skills` 字段：指向 plugin 内置 skill 目录（用于 plugin 自己提供的 skill）。
