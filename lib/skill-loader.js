...
class SkillLoader {
  ...
  async loadAll(workspace) {
    ...
    const normalizedSkillsRoot = ensureTrailingSep(resolvedSkillsRoot);
    ...
    for (const entry of entries) {
      ...
      try {
        // 加载前拒绝符号链接与越界真实路径，避免后续编辑命中任意文件。
        assertSafeLoadedSkill(normalizedSkillsRoot, skillDir, skillPath);
...
function parseFrontmatter(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (key === 'triggers' || key === 'tags' || key === 'actions') {
      const items = [];
...
function assertSafeLoadedSkill(normalizedSkillsRoot, skillDir, skillPath) {
  rejectSymlinkSync(skillDir);
  rejectSymlinkSync(skillPath);

  const resolvedDir = fs.realpathSync(skillDir);
  const resolvedFile = fs.realpathSync(skillPath);
  if (!ensureTrailingSep(resolvedDir).startsWith(normalizedSkillsRoot) ||
      !resolvedFile.startsWith(normalizedSkillsRoot)) {
    throw new Error('skill path escapes workspace/skills');
  }
}

function rejectSymlinkSync(targetPath) {
